...
(function(){
  ...
  const state={answers:Array(15).fill(null),current:0,screen:'intro',own:true,result:null,field:'전체'};
  ...
  function remember(hash){try{history.replaceState({lab:{version:VERSION,answers:state.answers,current:state.current,screen:state.screen,own:state.own}},'',location.pathname+location.search+(hash===undefined?location.hash:hash));}catch(e){/* file:// browsers may restrict history changes; sharing still encodes state. */}}
  function show(screen,focusId){document.body.classList.toggle('intro-active',screen==='intro');state.screen=screen;['intro','quiz','review','result'].forEach(id=>$(id).hidden=id!==screen);window.scrollTo(0,0);if(focusId)$(focusId).focus({preventScroll:true});}
  ORDER.forEach(t=>{const item=node('div','crew-item');item.append(character(t),node('span','',TYPES[t].short));$('crew').append(item);});
  function start(){state.answers=Array(15).fill(null);state.current=0;state.own=true;state.result=null;state.field='전체';$('entry-message').hidden=true;renderQuestion();remember('');}
  function renderQuestion(){const q=QUESTIONS[state.current];$('counter').textContent=(state.current+1)+' / 15';$('progress-fill').style.width=(state.current/15*100)+'%';document.querySelector('.progress').setAttribute('aria-valuenow',state.current);$('question').textContent=q.prompt;$('previous').disabled=state.current===0;$('previous').textContent='← 이전';$('saved-note').textContent='';$('choices').replaceChildren();const index=state.current;
    q.options.forEach((o,i)=>{const b=node('button','choice');b.type='button';b.dataset.option=o.id;b.setAttribute('aria-pressed',String(state.answers[index]===o.id));const tag=node('span','choice-tag',i===0?'A':'B');tag.setAttribute('aria-hidden','true');b.append(tag,node('span','',o.text));b.addEventListener('click',()=>{
      ...
      if(index===14){renderReview();}else{state.current++;renderQuestion();}
      remember('');
    });$('choices').append(b);});show('quiz','question');
  }
  function renderReview(){if(!logic.validate(state.answers)){const missing=state.answers.findIndex((id,i)=>!QUESTIONS[i].options.some(o=>o.id===id));state.current=Math.max(0,missing);renderQuestion();return;}show('review','review-title');}
  ...
  function renderResult(){const r=logic.score(state.answers);state.result=r;state.field='전체';$('share-banner').hidden=state.own;$('result-title').replaceChildren(document.createTextNode('나의 관심별'),document.createElement('br'),document.createTextNode('전공 탐색 결과'));$('result-chars').replaceChildren(...r.interests.map(character));$('interest-title').textContent=r.interests.map(t=>TYPES[t].short).join(' + ');$('interest-description').textContent=r.interests.length>2?'여러 활동에 관심이 함께 나타났어요. 아래에서 내 선택을 돌아보고, 더 궁금한 배움을 찾아보세요.':'이 활동들을 다른 활동보다 더 자주 골랐어요. 어떤 선택이 모였는지 함께 돌아볼까요?';$('tie-note').textContent=r.tied?'같은 횟수로 고른 관심은 함께 담았어요. 표시된 순서가 우열을 뜻하지 않아요.':'이번 15개 선택에서 보인 경향이에요. 잘하는 일이나 앞으로의 가능성을 정하는 결과는 아니에요.';
  ...
  function majorIcon(m){const badge=node('span','major-icon');badge.dataset.field=m.field;badge.setAttribute('aria-hidden','true');const s=document.createElementNS('http://www.w3.org/2000/svg','svg');s.setAttribute('viewBox','0 0 24 24');s.setAttribute('class','line-icon');const u=document.createElementNS(s.namespaceURI,'use');u.setAttribute('href','#ic-'+(window.LAB_MAJOR_ICONS[m.id]||'book'));s.append(u);badge.append(s);return badge;}
  function renderMajors(){const selected=state.result.candidates.filter(m=>state.field==='전체'||m.field===state.field).slice().sort((a,b)=>a.name.localeCompare(b.name,'ko'));$('major-list').replaceChildren();selected.forEach(m=>{const b=node('button','major-card');b.dataset.major=m.id;b.append(majorIcon(m),node('span','major-field',m.field+' · '+m.group),node('h3','',m.name),node('p','major-summary',m.learn.split('. ')[0]+(m.learn.includes('. ')?'.':'')));const tags=node('div','major-tags');m.interests.forEach(t=>tags.append(node('span','',TYPES[t].short)));b.append(tags,node('span','major-link','배우는 내용 살펴보기 ↗'));b.addEventListener('click',()=>openMajor(m));$('major-list').append(b);});}
  function openMajor(m){$('major-detail-icon').replaceChildren(majorIcon(m));$('major-field').textContent=m.field+' · '+m.group;$('major-title').textContent=m.name;$('major-learn').textContent=m.learn;$('major-why').textContent='“'+TYPES[m.interests[0]].name+'”와 “'+TYPES[m.interests[1]].name+'” 관심에 연결해 둔 전공이에요. 위의 배우는 내용에서 내가 해보고 싶은 활동을 찾아보세요.';$('major-careers').replaceChildren(...m.careers.map(c=>node('span','',c)));$('major-dialog').showModal();}
  function allAnswers(){if(!state.result)return;$('all-answers').replaceChildren();QUESTIONS.forEach((q,i)=>{const box=node('div','all-answer'),o=q.options.find(o=>o.id===state.answers[i]);box.append(node('p','',(i+1)+'. '+q.prompt),node('p','',o.text));$('all-answers').append(box);});$('answers-dialog').showModal();}
  ...
  function saveImage(){try{
    ...
    block('나의 관심별 전공 탐색',58,'#c8ff4d',22,800);block('이번 15개 선택에서 더 자주 고른 활동',30,'#b4bac4');block(r.interests.map(t=>TYPES[t].short).join(' + '),48,'#f4f5f1',36,750);
  ...
  $('start').addEventListener('click',start);$('previous').addEventListener('click',()=>{if(state.current>0){state.current--;renderQuestion();remember('');}});$('ready-previous').addEventListener('click',()=>{state.current=QUESTIONS.length-1;renderQuestion();remember('');});$('finish').addEventListener('click',()=>{if(!logic.validate(state.answers))return;state.own=true;renderResult();remember(logic.encode(state.answers));});$('view-answers').addEventListener('click',allAnswers);$('restart').addEventListener('click',()=>{state.answers=Array(15).fill(null);state.result=null;state.own=true;state.current=0;$('entry-message').hidden=true;show('intro');remember('');});$('copy').addEventListener('click',share);$('copy-link').addEventListener('click',copy);$('save-image').addEventListener('click',saveImage);$('close-share').addEventListener('click',()=>$('share-dialog').close());$('close-image').addEventListener('click',()=>$('image-dialog').close());
  ...
  function boot(){const saved=history.state&&history.state.lab;$('entry-message').hidden=true;
    ...
    if(saved&&saved.version===VERSION&&Array.isArray(saved.answers)&&saved.answers.length===15&&saved.answers.every((id,i)=>id===null||QUESTIONS[i].options.some(o=>o.id===id))&&['quiz','review'].includes(saved.screen)){
      state.answers=saved.answers;state.current=Math.max(0,Math.min(14,saved.current||0));state.own=true;if(saved.screen==='review'&&logic.validate(state.answers))renderReview();else renderQuestion();return;
    }show('intro');
  }
